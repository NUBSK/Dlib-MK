...
<xsl:stylesheet
    ...
    <xsl:template match="dim:dim" mode="itemSummaryView-DIM">
        <div class="item-summary-view-metadata" itemscope="" itemtype="http://schema.org/CreativeWork">
    ...
    <xsl:template name="itemSummaryView-DIM-fields">
      ...
      <xsl:choose>
	                    ...
	                    <xsl:choose>
	                        <xsl:when test="dim:field[@element='contributor'][@qualifier='author']">
	                            <xsl:for-each select="dim:field[@element='contributor'][@qualifier='author']">
                                        ...
                                        <a>
                                            <xsl:attribute name="href"><xsl:value-of select="concat($context-path,'/browse?value=')"/><xsl:copy-of select="."/>&amp;type=author</xsl:attribute>
          ...
          <!-- SUBJECT row -->
          <!-- identifier.uri row -->
          <xsl:when test="$clause = 7 and (dim:field[@element='subject'  and not(@qualifier) and descendant::text()])">
                    <div class="simple-item-view-description">
                  <span class="bold"><i18n:text>xmlui.dri2xhtml.METS-1.0.item-uri</i18n:text>:</span>
                  <span itemprop="keywords">
                    <xsl:for-each select="dim:field[@element='subject'  and not(@qualifier)]">
                        <a>
                            <xsl:attribute name="href"><xsl:value-of select="concat($context-path,'/browse?value=')"/><xsl:copy-of select="."/>&amp;type=subject</xsl:attribute><xsl:value-of select="text()"/>
                        </a>
